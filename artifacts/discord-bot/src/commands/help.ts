    ...
    fields: [
      {
        ...
        value: [
          ...
          '`th/lexg` *lastexportgrab* — Re-apply last `th/ihtx` export to a new attachment (Python bot)',
        ].join('\n'),
      },
      {
        ...
        value: [
          ...
          '**Distortion:** `mirror=<deg|preset>` `zoom=<amt>` `ripple=spd|freq|amp|phase` `pan=px|py` `tile=tx|ty` `pinch&punch=str;r;cx;cy` `shake=<h>|<v>` `wave=<preset>` `wave=custom:<hSpd>|<hFreq>|<hAmp>|<hPhase>|<vSpd>|<vFreq>|<vAmp>|<vPhase>`',
          '**Wave presets:** `largeWave` `mediumWave` `smallWave` `horizontalOnly` `verticalOnly` — also usable as standalone `th/wave <preset>`',
      ...
      },
      {
        ...
        value: [
          ...
          '`th/repeat [n]` *rep loop* — Repeat a video/GIF/audio N times (default 2, max 10)',
          ...
          '`th/pipetest <effect>` *pt* — One-shot pipe effect: `wave=largeWave`, `wave=custom:<params>`, `gradientmap <stops>`',
        ].join('\n'),
      },
    ],
  });
