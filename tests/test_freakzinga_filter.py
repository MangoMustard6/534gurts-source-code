...
def _build_filter_complex(lut_path: str, font_path: str, w: int, h: int, use_frei0r: bool) -> str:
    """Mirror the graph built by _run_freakzinga_test_effect for testing."""
    if use_frei0r:
        mirror_segment = (
...
def _run_graph(filter_complex: str, label: str) -> bool:
    cmd = [
    ...
    if result.returncode != 0:
        print(f"FAIL: {label} filter graph did not initialize")
        print(result.stderr[-1500:])
        return False
    print(f"PASS: {label} filter graph accepted by FFmpeg")
    return True
...
def main() -> int:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        w, h = 1920, 1080
        frei0r_available = _frei0r_mirr0r_available()
        tests = [(_build_filter_complex(lut_path, font_path, w, h, False), "native-mirror fallback")]
        if frei0r_available:
            tests.append((_build_filter_complex(lut_path, font_path, w, h, True), "frei0r=mirr0r path"))
        else:
            print("NOTE: frei0r=mirr0r plugin not available; skipping exact frei0r path")

        for graph, label in tests:
            if not _run_graph(graph, label):
                return 1
